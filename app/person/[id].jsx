...
const MovieOne = () => {
    ...
    return (
        <SafeAreaView style={{ backgroundColor: Colors.dark.background, color: Colors.dark.text, flex: 1, padding: 10 }}>
            <View style={{ backgroundColor: Colors.dark.background, color: Colors.dark.text, flex: 1 }}>
                <ScrollView showsVerticalScrollIndicator={false}>
                    <View style={styles.profileImgParent}>
                        {
                            people?.profile_path ?
                                <Image source={{ uri: `https://image.tmdb.org/t/p/w500${people?.profile_path}` }} style={styles.profileImage} />
                                :
                                <FontAwesome name="user" size={styles.profileImage.width} color="#ccc" />

                        }
                    </View>
                    <Text style={styles.title}>{people?.name}</Text>
                    <Text style={styles.birthPlace}>{people?.place_of_birth || 'Unknown birth place'}</Text>
                    <View style={styles.basicInfo}>
                        ...
                        <View style={styles.subBasicInfo}>
                            ...
                            <Text style={styles.subBasicInfoBody}>{getDate(people?.birthday) || 'Unknown'}</Text>
                        </View>
                        <View style={styles.subBasicInfo}>
                            <Text style={styles.subBasicInfoHeading}>Known for</Text>
                            <Text style={styles.subBasicInfoBody}>{people?.known_for_department}</Text>
                        </View>
                        <View style={[styles.subBasicInfo, { borderRightWidth: 0 }]}>
                            <Text style={styles.subBasicInfoHeading}>Popularity</Text>
                            <Text style={styles.subBasicInfoBody}>{Number(people?.popularity).toPrecision(2)}%</Text>
                        </View>
                    </View>
                    <Text style={styles.bioHeading}>Biography</Text>
                    <Text style={styles.bioInfo}>{people?.biography || 'Unknown'}</Text>
                    <ScrollView style={{ marginBottom: 20 }}>
                        {renderCategory('Movies', true, { fetching: loading, movies: movies })}
                    </ScrollView>
                </ScrollView >
            </View >
        </SafeAreaView>
