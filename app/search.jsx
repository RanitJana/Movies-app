import { View, ActivityIndicator, Text, StyleSheet, TextInput, TouchableOpacity, Image, FlatList } from 'react-native';
        ...
        <SafeAreaView style={[{ flex: 1, padding: 20 }, styles.parent]}>
            <View>
                <TextInput
                    style={styles.input}
                    ref={inputRef}
                    onChangeText={(text) => { setText(text); handleSearch(text); }}
            ...
            <View style={{ paddingVertical: 20, paddingBottom: 30 }}>
                {isLoading ? (
                    ...
                    <FlatList
                        ...
                        contentContainerStyle={{ alignItems: 'center', gap: 20, paddingBottom: 20 }}
                        numColumns={2}
                        onEndReached={loadMoreMovies}
                        onEndReachedThreshold={0.5}
                        ListFooterComponent={isAdditionalLoading && <ActivityIndicator size="large" color="orange" />}
                        showsVerticalScrollIndicator={false}
                    />
                )}
                {!isLoading && isLetter && movie.length === 0 && (
                    <View style={{ justifyContent: 'center', alignItems: 'center', width: '100%', height: '100%' }}>
                        <Image source={require('../assets/images/no-results.png')} style={{ width: 100, height: 100 }} />
